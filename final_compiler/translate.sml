  ...
  let
    ...
    fun initField ([], stmlist, index) = stmlist
      | initField (curr_exp::l, stmlist, index) =
        ...
        in
          initField (l, stmlist @ [statement], index + 1)
        end
  in
    Ex(T.ESEQ(seq ([T.MOVE(T.TEMP ptr, F.externalCall("tig_allocRecord", [T.CONST(fields_len * F.wordsize)]))] @ initField(fields, [], 0)), T.TEMP ptr))
  end
...
fun procEntryExit ({level=TOP, body=_}) = (ErrorMsg.error 0 "procEntryExit for a function declared in outermost level "; ())
  | procEntryExit ({level=Level({parent, frame:F.frame}, unique), body=body}) =
    let
      val body' = unEx body
    in
      fraglist := !fraglist @ [F.PROC{body=F.procEntryExit1(frame, T.MOVE(T.TEMP F.V0, body')), frame=frame}]
    end
