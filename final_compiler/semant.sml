...
struct
  ...
  fun transExp(venv, tenv, loop_end_label:(Temp.label option), level:Tr.level) =
	  ...
	  | trexp (A.ForExp{var, escape, lo, hi, body, pos}) =
	    ...
	    in
              {exp=Tr.transFOR(Tr.transSIMPLEVAR(access, level), lo_val, hi_val, body_exp, label_end), ty=T.UNIT}
	    end
  ...
  fun transProg(exp:A.exp) =
      let
        ...
        val tig_main_level = Tr.newLevel({parent=Tr.outermost, name=Temp.namedlabel "tig_main",formals=[]})
