structure Printtree :
          sig
            ...
            val printProg : MipsFrame.frag list -> unit
          end =
struct
...
fun printProg (frag_lst:MipsFrame.frag list) : unit =
    let
      ...
      fun indent 0 = ()
        | indent i = (say " "; indent(i-1))
      fun printFrag (F.STRING(label, str)) = (say (Symbol.name(label)); indent 4; say "\"";say str; sayln "\"")
        | printFrag (F.PROC{body, frame}) = (sayln "==========================="; printtree(os, body))
      val lst = map printFrag frag_lst
    in
      ()
    end
end
